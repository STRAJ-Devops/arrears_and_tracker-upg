...
class MaturityLoanController extends Controller
{
    public function group_by(Request $request)
    {
        ...
        dd($currentMonthYear, $nextMonthYear);

        // Check if request has group as parameter
        if ($request->has('group')) {
            if ($request->group == 'staff_id') {
                $arrears = Arrear::where('maturity_date', 'LIKE', $currentMonthYear)->groupBy('staff_id')->get();
                $groupKey = 'staff_id';
                $nameField = 'officer';
                $nameAttribute = 'names';
            } else if ($request->group == 'branch_id') {
                $arrears = Arrear::where('maturity_date', 'LIKE', $currentMonthYear)->groupBy('branch_id')->get();
                $groupKey = 'branch_id';
                $nameField = 'branch';
                $nameAttribute = 'branch_name';
            } else if ($request->group == 'region_id') {
                $arrears = Arrear::where('maturity_date', 'LIKE', $currentMonthYear)->groupBy('region_id')->get();
                $groupKey = 'region_id';
                $nameField = 'region';
                $nameAttribute = 'region_name';
            } else if ($request->group == 'loan_product') {
                $arrears = Arrear::where('maturity_date', 'LIKE', $currentMonthYear)->groupBy('product_id')->get();
                $groupKey = 'product_id';
                $nameField = 'product';
                $nameAttribute = 'product_name';
            } else if ($request->group == 'gender') {
                $arrears = Arrear::where('maturity_date', 'LIKE', $currentMonthYear)->groupBy('gender')->get();
                $groupKey = 'gender';
                $nameField = 'gender';
                $nameAttribute = "None";
            } else if ($request->group == 'district') {
                $arrears = Arrear::where('maturity_date', 'LIKE', $currentMonthYear)->groupBy('district_id')->get();
                $groupKey = 'district_id';
                $nameField = 'district';
                $nameAttribute = 'district_name';
            } else if ($request->group == 'sub_county') {
                $arrears = Arrear::where('maturity_date', 'LIKE', $currentMonthYear)->groupBy('subcounty_id')->get();
                $groupKey = 'subcounty_id';
                $nameField = 'sub_county';
                $nameAttribute = 'subcounty_name';
            } else if ($request->group == 'village') {
                $arrears = Arrear::where('maturity_date', 'LIKE', $currentMonthYear)->groupBy('village_id')->get();
                $groupKey = 'village_id';
                $nameField = 'village';
                $nameAttribute = 'village_name';
            } else if ($request->group == 'age') {
                $arrears = Arrear::where('maturity_date', 'LIKE', $currentMonthYear)->get()->groupBy(function ($arrear) {
                ...
                $arrears = Arrear::where('maturity_date', 'LIKE', $currentMonthYear)->whereRaw('(principal_arrears+outstanding_interest)>0')->get()->groupBy('customer_id');
                $groupKey = 'client_id';
                $nameField = 'customer';
                $nameAttribute = 'names';
            } else {
                // Default to group by staff_id
                $arrears = Arrear::where('maturity_date', 'LIKE', $currentMonthYear)->where('maturity_date', 'LIKE', $currentMonthYear)->groupBy('staff_id');
                $groupKey = 'staff_id';
                $nameField = 'officer';
                $nameAttribute = 'names';
            }
