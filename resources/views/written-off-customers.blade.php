                    ...
                    success: function(response) {
                        ...
                        if (response.status === 'success' && response.data.length > 0) {
                            ...
                            function toNumberSafe(val) {
                                if (!val) return 0; // handles null, undefined, empty string
                            ...
                            response.data.forEach(function(customer) {
                                ...
                                const principalWOF = toNumberSafe(customer.principal_written_off);
                                const interestWOF = toNumberSafe(customer.interest_written_off);
                                const totalWOF = principalWOF + interestWOF;

                                const principalPaid = toNumberSafe(customer.principal_paid);
                                const interestPaid = toNumberSafe(customer.interest_paid);
                                const totalPaid = principalPaid + interestPaid;
