$(document).ready(function () {
            ...
            success: function (response) {
                ...
                $.each(data, function (index, item) {
                    if (group === 'age') {
                        ...
                        var numberOfCommentsHtml = '<button class="btn btn-sm btn-outline-primary view-comments" data-arrear-id="' + item.arrear_id + '">' + item.number_of_comments.toLocaleString() + '</button>';
                        var row = [
                            //item.arrear_id,
                            item.names,
                            item.phone_number,
                            ...
                            item.total_interest_arrears.toLocaleString(), // Pad with comma after every three digits
                            item.total_arrears.toLocaleString(), // Pad with comma after every three digits
                            '<button class="btn btn-primary comment-button" data-arrear-id="' + item.arrear_id + '"><i class="fa fa-commenting" aria-hidden="true"></i></button>'
                        ];
                    } else {
                        var row = [
                            item.group_key,
    ...
    function drawTable() {
        ...
        $('#arrears tbody').on('click', '.comment-button', function () {
            ...
            var arrearId = $(this).data('arrear-id');
            // Set the arrear id value in the hidden input field
            $('#arrear_id').val(arrearId);
            // Show the modal with the comment box or perform any other action
            $('#commentModal').modal('show');

        });

        // Function to fetch comments for a specific arrear_id
        function fetchComments(arrearId) {
        ...
        $('#arrears tbody').on('click', '.view-comments', function () {
            // Retrieve the arrear id from the data attribute of the clicked button
            var arrearId = $(this).data('arrear-id');
            // Call the fetchComments function with the retrieved arrearId
            fetchComments(arrearId);
            // Show the modal with the comment box or perform any other action
        ...
        $('#submitComment').click(function () {
            // Retrieve the arrear_id value from the hidden input field
            var arrearId = $('#arrear_id').val();
